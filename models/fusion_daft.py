...
class DAFTBlock(nn.Module):
    ...
    def forward(self, feature_map, x_aux):
        ...
        if self.scale == self.shift:
            v_scale, v_shift = torch.split(attention, self.split_size, dim=1) # bs, 512 -> bs, 256 & bs, 256
            v_scale = v_scale.view(v_scale.size()[0], 1, v_scale.size()[1]).expand_as(x_aux) # bs, 256 -> bs, seq_len, 256
