...
class DrFuseModel(nn.Module):
    def __init__(self, hidden_size, num_classes, ehr_dropout, ehr_n_layers, ehr_n_head,
                 note_model='swin_s', logit_average=False, args=None):
        super().__init__()
        ...
        self.note_model_shared_fc = nn.Linear(in_features=args.d_txt, out_features=hidden_size)
        self.note_model_spec = NoteModel(args)
        self.note_model_spec_fc = nn.Linear(in_features=args.d_txt, out_features=hidden_size)
        ...
        self.note_model_linear = nn.Linear(in_features=hidden_size, out_features=num_classes)
    ...
    def forward(self, x, token, mask, seq_lengths, pairs, grl_lambda):
        ...
        attn_input = torch.stack([feat_ehr_distinct, feat_avg_shared, feat_note_distinct], dim=1)
