...
class Fusion(nn.Module):
    ...
    def forward(self, x, seq_lengths=None, token=None, mask=None, pairs=None):
        if self.args.fusion_type == 'uni_note':
            return self.forward_uni_note(x, seq_lengths=seq_lengths, token=token, mask=mask)
        elif self.args.fusion_type in ['joint', 'early', 'late_avg', 'unified']:
            return self.forward_fused(x, seq_lengths=seq_lengths, token=token, mask=mask, pairs=pairs)
        elif self.args.fusion_type == 'uni_ehr':
            return self.forward_uni_ehr(x, seq_lengths=seq_lengths, token=token, mask=mask)
        elif self.args.fusion_type == 'lstm':
            return self.forward_lstm_fused(x, seq_lengths=seq_lengths, token=token, mask=mask, pairs=pairs)
        elif self.args.fusion_type == 'uni_ehr_lstm':
            return self.forward_lstm_ehr(x, seq_lengths=seq_lengths, token=token, mask=mask, pairs=pairs)
    ...
    def forward_lstm_fused(self, x, seq_lengths=None, token=None, mask=None, pairs=None):
