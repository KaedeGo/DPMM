...
import torch
import numpy as np
from models.loss import CosineLoss, KLDivLoss, DirichletProcess3DLoss
import torch.nn.functional as F
from models.multimodal_fusion import CrossAttentionFusion3D
...
class DP_Fusion(nn.Module):
    def __init__(self, args, ehr_model, cxr_model, note_model):
        ...
        self.dp_loss = DirichletProcess3DLoss(K=args.K, M=3, rho_scale=args.rho_scale)

        self.lstm_fused_cls = nn.Sequential(
            nn.Linear(lstm_out, target_classes), nn.Sigmoid()
        )

        self.cross_attention_fusion = CrossAttentionFusion3D(in_ts_size=lstm_in, in_cxr_size=lstm_in, in_note_size=lstm_in)
    ...
    def forward(self, x, seq_lengths=None, img=None, token=None, mask=None, pairs=None):
        ...
        dp_loss = self.dp_loss(ehr_feats, projected_img, projected_note)
        ...
        if self.args.dp_fuse_type == "lstm":
            ...
            if len(ehr_feats.shape) == 1:
                feats = ehr_feats[None,:]
            fusion_feat = self.cross_attention_fusion(ehr_feats, projected_img, projected_note)
