...
class Fusion(nn.Module):
    ...
    def forward(self, x, seq_lengths=None, img=None, token=None, mask=None, pairs=None):
        if self.args.fusion_type in ["joint", "early", "late_avg", "unified"]:
            return self.forward_fused(
                x, seq_lengths=seq_lengths, img=img, token=token, mask=mask, pairs=pairs
            )
    ...
    def forward_fused(
        self, x, seq_lengths=None, img=None, token=None, mask=None, pairs=None
    ):
    ...
    def forward_lstm_fused(
        ...
        cxr_feats = self.projection_img(cxr_feats)
        note_feats = self.projection_note(note_feats)

        cxr_feats[list(~np.array(pairs))] = 0
        if len(ehr_feats.shape) == 1:
            feats = ehr_feats[None, None, :]
            feats = torch.cat(
                [feats, cxr_feats[:, None, :], note_feats[:, None, :]], dim=1
            )
        else:
