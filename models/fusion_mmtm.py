...
class MMTM(nn.Module):
    ...
    def forward(self, visual, skeleton):
        squeeze_array = []
        visual_view = visual.view(visual.shape[:2] + (-1,)) # bs, 512, 7, 7 -> bs, 512, 49
        ...
        vis_out = vis_out.view(vis_out.shape + (1,) * dim_diff) # bs, 512 -> bs, 512, 1, 1

        dim_diff = len(skeleton.shape) - len(sk_out.shape)
        sk_out = sk_out.view(sk_out.shape[0], 1 , sk_out.shape[1]) # bs, 256 -> bs, 1, 256
...
class FusionMMTM(nn.Module):
    ...
    def forward(self, ehr, seq_lengths=None, img=None, n_crops=0, bs=16):
        ...
        ehr_unpacked, _ = torch.nn.utils.rnn.pad_packed_sequence(ehr, batch_first=True) # bs, seq_len, feats_dim(256)
        ...
        projected = self.projection(ehr_feats) # bs, 512
        loss = self.kl_loss(cxr_feats, projected)

        feats = torch.cat([projected, cxr_feats], dim=1) # bs, 1024
