...
class DrFuseModel(nn.Module):
    def __init__(self, hidden_size, num_classes, ehr_dropout, ehr_n_layers, ehr_n_head,
        ...
        self.cxr_model_shared_fc = nn.Linear(in_features=resnet.fc.in_features, out_features=hidden_size)
        ...
        self.cxr_model_spec_fc = nn.Linear(in_features=resnet.fc.in_features, out_features=hidden_size)
    ...
    def forward(self, x, img, seq_lengths, pairs, grl_lambda):
        feat_ehr_shared, feat_ehr_distinct, pred_ehr = self.ehr_model(x, seq_lengths) # batch_size x hidden_size(256)
        ...
        feat_cxr_shared = self.cxr_model_shared_fc(feat_cxr_shared) # batch_size x hidden_size(256)
        feat_cxr_distinct = self.cxr_model_spec_fc(feat_cxr_distinct) # batch_size x hidden_size(256)

        # get shared feature
        pred_cxr = self.cxr_model_linear(feat_cxr_distinct).sigmoid()

        feat_ehr_shared = self.shared_project(feat_ehr_shared) # batch_size x hidden_size(256)
        feat_cxr_shared = self.shared_project(feat_cxr_shared) # batch_size x hidden_size(256)

        pairs = pairs.unsqueeze(1)

        h1 = feat_ehr_shared # batch_size x hidden_size
        h2 = feat_cxr_shared
        term1 = torch.stack([h1+h2, h1+h2, h1, h2], dim=2) # batch_size x hidden_size x 4
        term2 = torch.stack([torch.zeros_like(h1), torch.zeros_like(h1), h1, h2], dim=2) # batch_size x hidden_size x 4
        feat_avg_shared = torch.logsumexp(term1, dim=2) - torch.logsumexp(term2, dim=2) # batch_size x hidden_size
        ...
        attn_input = torch.stack([feat_ehr_distinct, feat_avg_shared, feat_cxr_distinct], dim=1) # batch_size x 3 x hidden_size(256)
        qkvs = self.attn_proj(attn_input) # batch_size x (2+num_classes)*hidden_size(256)
        q, v, *k = qkvs.chunk(2+self.num_classes, dim=-1)

        # compute query vector
        q_mean = pairs * q.mean(dim=1) + (1-pairs) * q[:, :-1].mean(dim=1) # batch_size x hidden_size(256)

        # compute attention weighting
        ks = torch.stack(k, dim=1) # batch_size x 1 x 2+num_classes x hidden_size(256)
        attn_logits = torch.einsum('bd,bnkd->bnk', q_mean, ks) # batch_size x 1 x 2+num_classes
        attn_logits = attn_logits / math.sqrt(q.shape[-1]) # batch_size x 1 x 2+num_classes
