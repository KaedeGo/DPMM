...
def plot_types_bar():
    ...
    single_aurocs_errors = np.abs(
        np.array([(0.732, 0.789), (0.724, 0.773), (0.693, 0.741)]).T - single_aurocs
    )
    single_auprcs = [0.432, 0.458, 0.487]
    single_auprcs_errors = np.abs(
        np.array([(0.386, 0.486), (0.413, 0.506), (0.448, 0.530)]).T - single_auprcs
    )

    multi_aurocs = [0.772, 0.800, 0.745]
    multi_aurocs_errors = np.abs(
        np.array([(0.744, 0.800), (0.776, 0.823), (0.723, 0.768)]).T - multi_aurocs
    )
    multi_auprcs = [0.433, 0.565, 0.512]
    multi_auprcs_errors = np.abs(
        np.array([(0.386, 0.486), (0.516, 0.614), (0.473, 0.555)]).T - multi_auprcs
    )
    ...
    plt.bar(xs1, single_aurocs, color="#FF1F5B", width=barWidth, label="Uni-modal")
    plt.bar(xs2, multi_aurocs, color="#009ADE", width=barWidth, label="Multi-modal")
...
def plot_loss():
    ...
    keys = ["loss train joint", "loss val joint", "auroc val joint"]
