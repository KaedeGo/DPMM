...
class DrFuseTrainer(Trainer):
    ...
    def _disentangle_loss_jsd(self, model_output, pairs, mode="train"):
        ...
        loss_sim_note = self._masked_abs_cos_sim(
            model_output["feat_note_shared"], model_output["feat_note_distinct"], pairs
        )
    ...
    def _compute_prediction_losses(self, model_output, y_gt, pairs, mode="train"):
        ...
        loss_pred_note = self._compute_masked_pred_loss(
            model_output["pred_note"], y_gt, pairs
        )
    ...
    def _compute_and_log_loss(self, model_output, y_gt, pairs, mode="train"):
        ...
        loss_pred_final, loss_pred_ehr, loss_pred_note, loss_pred_shared = (
            prediction_losses
        )
        ...
        raw_pred_loss_note = F.binary_cross_entropy(
            model_output["pred_note"].data, y_gt, reduction="none"
        )
        ...
        attn_ehr, attn_shared, attn_note = (
            attn_weights[:, :, 0],
            attn_weights[:, :, 1],
            attn_weights[:, :, 2],
        )
        ...
        loss_attn1 = pairs * F.margin_ranking_loss(
            attn_note, attn_ehr, note_overweights_ehr, reduction="none"
        )
        ...
        shared_overweights_note = (
            2 * (raw_pred_loss_shared < raw_pred_loss_note).float() - 1
        )
        loss_attn3 = pairs * F.margin_ranking_loss(
            attn_shared, attn_note, shared_overweights_note, reduction="none"
        )
    ...
    def train_epoch(self):
        ...
        for i, (x, token, mask, y_ehr, y_note, seq_lengths, pairs) in enumerate(
            ...
            if (
                ...
                y = y.unsqueeze(1)
            out = self.model(
                x, token, mask, seq_lengths, pairs, self._get_alignment_lambda()
            )
            ...
            if self.args.attn_fusion == "avg":
                perd_final = (
                    out["pred_ehr"] + out["pred_note"] + out["pred_shared"]
                ) / 3
