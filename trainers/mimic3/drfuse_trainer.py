...
from models.mimic3.drfuse import DrFuseModel
...
class DrFuseTrainer(Trainer):
    ...
    test_dl=None
        ...
        self.optimizer = optim.Adam(self.model.parameters(), args.lr, weight_decay=self.args.wd)
        self.load_state()
    ...
    def _disentangle_loss_jsd(self, model_output, pairs, mode='train'):
        ...
        loss_sim_note = self._masked_abs_cos_sim(model_output['feat_note_shared'],
                                                model_output['feat_note_distinct'], pairs)
        ...
        jsd = self.jsd(model_output['feat_ehr_shared'].sigmoid(),
                       model_output['feat_note_shared'].sigmoid(), pairs)
    ...
    def _compute_prediction_losses(self, model_output, y_gt, pairs, mode='train'):
        ...
        loss_pred_note = self._compute_masked_pred_loss(model_output['pred_note'], y_gt, pairs)
        loss_pred_shared = self._compute_masked_pred_loss(model_output['pred_shared'], y_gt, ehr_mask)

        return loss_pred_final, loss_pred_ehr, loss_pred_note, loss_pred_shared

    def _compute_and_log_loss(self, model_output, y_gt, pairs, mode='train'):
        prediction_losses = self._compute_prediction_losses(model_output, y_gt, pairs, mode)
        loss_pred_final, loss_pred_ehr, loss_pred_note, loss_pred_shared = prediction_losses

        loss_prediction = (self.args.lambda_pred_shared * loss_pred_shared +
                           self.args.lambda_pred_ehr * loss_pred_ehr +
                           self.args.lambda_pred_note * loss_pred_note)
        ...
        raw_pred_loss_note = F.binary_cross_entropy(model_output['pred_note'].data, y_gt, reduction='none')
        ...
        attn_ehr, attn_shared, attn_note = attn_weights[:, :, 0], attn_weights[:, :, 1], attn_weights[:, :, 2]

        note_overweights_ehr = 2 * (raw_pred_loss_note < raw_pred_loss_ehr).float() - 1
        loss_attn1 = pairs * F.margin_ranking_loss(attn_note, attn_ehr, note_overweights_ehr, reduction='none')
        ...
        shared_overweights_note = 2 * (raw_pred_loss_shared < raw_pred_loss_note).float() - 1
        loss_attn3 = pairs * F.margin_ranking_loss(attn_shared, attn_note, shared_overweights_note, reduction='none')
    ...
    def train_epoch(self):
        ...
        for i, (x, token, mask, y_ehr, y_note, seq_lengths, pairs) in enumerate (self.train_dl):
            ...
            if self.args.task == 'in-hospital-mortality' or self.args.task == 'readmission':
                y = y.unsqueeze(1)
            out = self.model(x, token, mask, seq_lengths, pairs, self._get_alignment_lambda())
            ...
            if self.args.attn_fusion == 'avg':
                perd_final = (out['pred_ehr'] + out['pred_note'] + out['pred_shared']) / 3
        ...
        with torch.no_grad():
            for i, (x, token, mask, y_ehr, y_note, seq_lengths, pairs) in enumerate (dl):
                ...
                if self.args.attn_fusion == 'avg':
                    ...
                    pred_final =  out['pred_final']
                loss = self._compute_masked_pred_loss(pred_final, y, torch.ones_like(y[:, 0]))
                epoch_loss += loss.item()
                outPRED = torch.cat((outPRED, pred_final), 0)
                outGT = torch.cat((outGT, y), 0)
