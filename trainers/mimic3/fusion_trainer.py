...
from models.mimic3.fusion import Fusion
...
class FusionTrainer(Trainer):
    def __init__(self, 
        ...
        self.model = Fusion(args, self.ehr_model, self.note_model ).to(self.device)
        ...
        self.epochs_stats = {'loss train': [], 'loss val': [], 'auroc val': [], 'loss copula train': [], 'loss copula val': [], 'loss align train': [], 'loss align val': []}
    ...
    def init_fusion_method(self):
        ...
        if 'uni_ehr' in self.args.fusion_type:
            self.freeze(self.model.note_model)
        elif 'uni_note' in self.args.fusion_type:
