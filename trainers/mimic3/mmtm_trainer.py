...
class MMTMTrainer(Trainer):
    def __init__(self, 
        ...
        self.optimizer_note = optim.Adam([{'params': self.model.note_model.parameters()}], args.lr, betas=(0.9, self.args.beta_1))
        self.optimizer_ehr = optim.Adam([{'params': self.model.ehr_model.parameters()}], args.lr, betas=(0.9, self.args.beta_1))
        ...
        print(self.loss)
        self.scheduler_visual = ReduceLROnPlateau(self.optimizer_note, factor=0.5, patience=10, mode='min')
        self.scheduler_ehr = ReduceLROnPlateau(self.optimizer_ehr, factor=0.5, patience=10, mode='min')

        self.best_auroc = 0
        self.best_stats = None
        self.epochs_stats = {'loss train note': [], 'loss train ehr': [], 'loss val note': [], 'loss val ehr': [], 'auroc val note': [], 'auroc val ehr': [], 'auroc val avg': [],
    ...
    def train_epoch(self):
        ...
        for i, (x, token, mask, y_ehr, y_note, seq_lengths, _) in enumerate (self.train_dl):
    ...
    def validate(self, dl, full_run=False):
        ...
        ret_note = self.computeAUROC(outGT.data.cpu().numpy(), outPRED_note.data.cpu().numpy(), 'validation_note')
    ...
    def eval(self):
        ...
        self.print_and_write(ret['note'] , isbest=True, prefix=f'{self.args.fusion_type} val', filename='results_val_note.txt')
        ...
        self.print_and_write(ret['note'] , isbest=True, prefix=f'{self.args.fusion_type} test', filename='results_test_note.txt')
    ...
    def train(self):
        ...
        for self.epoch in range(self.start_epoch, self.args.epochs):
            ...
            intrabest = max([ret['late']['auroc_mean'], ret['note']['auroc_mean'], ret['ehr']['auroc_mean'], ret['joint']['auroc_mean']])
            if self.best_auroc < intrabest:
                ...
                self.print_and_write(ret['note'], prefix='valnote', isbest=True, filename='results_val_note.txt')
                self.patience = 0
            else:
                self.patience+=1
        ...
        self.print_and_write(self.best_stats['note'] , isbest=True, filename='results_val_note.txt')
