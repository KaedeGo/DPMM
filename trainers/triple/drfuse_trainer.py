...
from models.triple.drfuse import DrFuseModel
...
class DrFuseTrainer(Trainer):
    ...
    def _compute_and_log_loss(self, model_output, y_gt, pairs, mode="train"):
        ...
        loss_prediction = (
            ...
            + self.args.lambda_pred_cxr * loss_pred_note
        )
        ...
        attn_ehr, attn_shared, attn_cxr, attn_note = (
            attn_weights[:, :, 0],
            attn_weights[:, :, 1],
            attn_weights[:, :, 2],
            attn_weights[:, :, 3],
        ...
        loss_attn4 = pairs * F.margin_ranking_loss(
            attn_note, attn_ehr, note_overweights_ehr, reduction="none"
        )
        loss_attn4 = loss_attn4.sum() / max(1e-6, loss_attn4[loss_attn4 > 0].numel())

        note_overweights_cxr = 2 * (raw_pred_loss_note < raw_pred_loss_cxr).float() - 1
        loss_attn5 = pairs * F.margin_ranking_loss(
            attn_note, attn_cxr, note_overweights_cxr, reduction="none"
        )
        ...
        loss_attn6 = pairs * F.margin_ranking_loss(
