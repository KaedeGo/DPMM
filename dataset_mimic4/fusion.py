...
class MIMIC_CXR_EHR(Dataset):
    def __init__(self, args, metadata_with_labels, ehr_ds, cxr_ds, split='train'):
        ...
        self.ehr_files_unpaired = sorted(list(set(self.ehr_files_all) - set(self.ehr_files_paired)))
        self.ehr_ds = ehr_ds
        self.cxr_ds = cxr_ds
        self.args = args
        self.split = split
        ...
        # elif split == 'val':
        #     self.data_ratio =  0.0
        print(f"split: {split}, ehr_files_all: {len(self.ehr_files_all)}, ehr_files_paired: {len(self.ehr_files_paired)}, cxr_files_all: {len(self.cxr_files_all)}, cxr_files_paired: {len(self.cxr_files_paired)}, ehr_files_unpaired: {len(self.ehr_files_unpaired)}")
    ...
    def __getitem__(self, index):
        if self.args.data_pairs == 'paired_ehr_cxr':
            ...
            if index < len(self.ehr_files_paired):
                ...
                ehr_data, labels_ehr = self.ehr_ds[self.ehr_files_unpaired[index-len(self.ehr_files_paired)]]
                cxr_data, labels_cxr = None, None
            return ehr_data, cxr_data, labels_ehr, labels_cxr
...
def load_cxr_ehr(args, ehr_train_ds, ehr_val_ds, cxr_train_ds, cxr_val_ds, ehr_test_ds, cxr_test_ds):
    ...
    train_dl = DataLoader(train_ds, args.batch_size, shuffle=True, collate_fn=my_collate, pin_memory=True, num_workers=0, drop_last=True)
    val_dl = DataLoader(val_ds, args.batch_size, shuffle=False, collate_fn=my_collate, pin_memory=True, num_workers=0, drop_last=False)
    test_dl = DataLoader(test_ds, args.batch_size, shuffle=False, collate_fn=my_collate, pin_memory=True, num_workers=0, drop_last=False)
...
'''
    Readmission:
    split: train, ehr_files_all: 18064, ehr_files_paired: 4287, cxr_files_all: 344529, cxr_files_paired: 4287, ehr_files_unpaired: 13777
    split: val, ehr_files_all: 2035, ehr_files_paired: 465, cxr_files_all: 9497, cxr_files_paired: 465, ehr_files_unpaired: 1570
    split: test, ehr_files_all: 4972, ehr_files_paired: 1179, cxr_files_all: 23069, cxr_files_paired: 1179, ehr_files_unpaired: 3793
