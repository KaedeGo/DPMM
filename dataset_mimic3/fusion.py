...
class MIMIC_NOTE_EHR(Dataset):
    def __init__(self, args, ehr_ds, note_ds, split='train'):
        ...
        # elif split == 'val':
        #     self.data_ratio =  0.0
        print(f"split: {split}, ehr_files_all: {len(self.files_all)}, ehr_files_paired: {len(self.files_paired)}")
    ...
    def __getitem__(self, index):
        if self.args.data_pairs == 'paired_ehr_note':
            ...
            if index < len(self.files_paired):
                ...
                ehr_data, labels_ehr = self.ehr_ds[self.files_unpaired[index-len(self.files_paired)]]
                text_token, atten_mask, labels_note = None, None, None
            return ehr_data, text_token, atten_mask, labels_ehr, labels_note
    ...
    def __len__(self):
        if 'paired' in self.args.data_pairs:
            ...
            return len(self.files_paired) + int(self.data_ratio * len(self.files_unpaired)) 
        

def load_note_ehr(args, ehr_train_ds, ehr_val_ds, note_train_ds, note_val_ds, ehr_test_ds, note_test_ds):
...
def my_collate(batch):
    ...
    token = torch.stack([torch.zeros((5, 512), dtype=torch.int64) if item[1] is None else item[1] for item in batch])
    mask = torch.stack([torch.zeros((5, 512), dtype=torch.int64) if item[2] is None else item[2] for item in batch])
    x, seq_length = pad_zeros(x)
    targets_ehr = np.array([item[3] for item in batch])
    targets_note = torch.stack([torch.tensor(0.) if item[4] is None else item[4] for item in batch])
...
if __name__ == "__main__":
    ...
    args.normalizer_state = "/disk1/fwu/myProjects/MedFuse/data_mimic3/ihm_ts.normalizer"
